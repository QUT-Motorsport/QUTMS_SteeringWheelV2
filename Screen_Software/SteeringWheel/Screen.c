...
void Screen_4Gray_Init(void)
{
    ...
    Screen_SendData(0xF7); //Something related to ram clearly, B/W and RED.
    ...
    Screen_SendData(0x80); // "A[7:0] = 80h Internal temperature sensor" is this what is being sent? UNDER 18

    Screen_SendCommand(SCREEN_CMD_WRITE_VCOM_REG); // set vcom value
    Screen_SendData(0x44); // "A[7:0] = 00h [POR] | 44h -1.7 94h -3.7" UNDER 2C
